...
% 8 numaralı sensor ARKA BACAK VE OK AŞAĞI (roll)
...
quat_log1 = quaternion.zeros(size(acc1,1),1);


ifilt = imufilter(SampleRate=148);
for ii=1:size(acc1,1)
    qimu1 = ifilt(acc1(ii,:),gyro1(ii,:));
    quat_log1(ii) = qimu1;
end
...
euler_angles1 = eulerd(quat_log1, "ZYX", "frame");
knee_angle = euler_angles1(:,3);
