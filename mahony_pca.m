...
for i=1:N-1
    dq(i) = q_results(i+1)*q_results_conj(i);
end
...
for i=1:N_sample
    eksen_log(i,:) = eksen_log(i,:)/norm(eksen_log(i,:));
    fi(i) = (eksen_log(i,:)*ana_eksen_n') .* abs(anlik_acilar_derece(i));
end
...
% Ham veriler
val_method3 = knee_angle-mean(knee_angle(1:148*10))-3;
